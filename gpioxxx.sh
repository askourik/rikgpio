...
logger "gpio.sh method=$method"
arr1=(${method// / })
mode1=${arr1[1]}
mode2=$(echo $mode1 | tr -d \") 
arr2=(${mode2//./ })
mode=${arr2[5]}
logger "gpio.sh mode=$mode"
...
if [ $mode == 'Manual' ]; then
 ...
 logger "gpio.sh numserv=$numserv"
 ...
 for i in "${!arr3[@]}"; do 
  ...
  if [ $i -gt 1 ];  then
    ...
    then
      sleep 5
      logger "gpio.sh modenew=$mode gpioservernew=$gpioserver"
      newdate=$(date) 
      logger "gpio.sh set NTP from $gpioserver = $newdate"
      logger "gpio.sh NTP write $mode $gpioserver to rikgpio.conf"
      echo $mode $gpioserver > /etc/rikgpio/rikgpio.conf
      sleep 1
      break
    fi
    logger "gpio.sh set NTP from $gpioserver failed, trying next"
    sleep 5
  fi
 done
